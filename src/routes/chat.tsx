...
const MOCK_MESSAGES_BY_ID: Record<number, Message[]> = {
      ...
      2: [
            ...
            },
            {
                  id: 2,
                  sender: 'Me',
                  text: 'Yes, let me upload them here.',
                  isMe: true,
                  time: 'Yesterday'
            },
            {
      ...
      3: [
            {
                  ...
                  text: 'Meeting in 5 mins. Are you ready?',
                  isMe: false,
                  time: 'Yesterday'
            },
            {
                  id: 2,
                  sender: 'Me',
                  text: 'Almost, just grabbing coffee.',
                  isMe: true,
                  time: 'Yesterday'
            }
      ]
...
function ChatComponent() {
      ...
      // Select friend based on URL or default to first
      ...
      // Sync URL if no ID is present initially
      useEffect(() => {
            if (!activeChatId) {
      ...
      const handleSendMessage = (e: React.FormEvent) => {
            ...
            // Here you would typically add the message to the state/backend
            setMessage('');
      };

      return (
            <div className="h-full flex gap-6 max-w-[1600px] mx-auto overflow-hidden">
                  {/* LEFT COLUMN: Friends List */}
                  <section
                        className="w-80 min-w-[300px] flex flex-col bg-bg-secondary border border-border rounded-xl failure-safe overflow-hidden max-lg:w-20 max-lg:min-w-[80px] transition-all"
                        aria-label="Friends list"
                  >
                        {/* Header/Search */}
                        <div className="p-4 border-b border-border">
                              <h2 className="text-lg font-bold text-text-primary mb-4 max-lg:hidden">Messages</h2>
                              <div className="relative">
                                    <input
                                          type="text"
                                          placeholder="Search messages..."
                                          className="w-full bg-bg-primary border border-border rounded-lg py-2 pl-9 pr-4 text-sm text-text-primary focus:outline-accent max-lg:hidden focus-visible:ring-2 focus-visible:ring-accent"
                        ...
                        <div className="flex-1 overflow-y-auto p-2 space-y-1">
                              {MOCK_FRIENDS.map(friend => (
                                    <button
                                          ...
                                          className={`w-full flex items-center gap-3 p-3 rounded-lg transition-colors text-left group focus-visible:outline-2 focus-visible:outline-accent
                ${selectedFriend.id === friend.id ? 'bg-accent/10' : 'hover:bg-bg-hover'}
              `}
                                          aria-current={selectedFriend.id === friend.id ? 'true' : undefined}
                                    >
                ...
                bg-linear-to-br from-accent to-accent-hover`}
                                    ...
                                    <div>
                                          <h3 className="font-bold text-text-primary">{selectedFriend.name}</h3>
                                          <p
                              ...
                              <button
                                    className="lg:hidden p-2 text-text-secondary hover:bg-bg-hover rounded-lg focus-visible:outline-2 focus-visible:outline-accent"
                                    onClick={() => setShowRightPanel(!showRightPanel)}
                                    aria-label={showRightPanel ? 'Hide files' : 'Show files'}
                        ...
                        {/* Messages */}
                        <div
                              className="flex-1 overflow-y-auto p-4 space-y-4 bg-bg-tertiary/30"
                              role="log"
                              aria-live="polite"
                        >
                ...
                `}
                        ...
                        <div className="p-4 border-t border-border bg-bg-secondary">
                              <form className="flex gap-3" onSubmit={handleSendMessage}>
                                    <button
                                          type="button"
                                          className="p-2 text-text-muted hover:text-accent transition-colors rounded-lg hover:bg-bg-hover focus-visible:outline-2 focus-visible:outline-accent"
                                    ...
                                    <input
                                          ...
                                          className="flex-1 bg-bg-primary border border-border rounded-xl px-4 py-2 text-sm focus:outline-accent focus-visible:ring-2 focus-visible:ring-accent"
                                    />
                                    <button
                                          type="submit"
                                          className="p-2 bg-accent text-white rounded-xl hover:bg-accent-hover transition-colors shadow-lg shadow-accent/20 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
        ...
        `}
                  ...
                  >
                        ...
                        <div className="flex-1 overflow-y-auto p-4">
                              <div className="space-y-3">
                                    {MOCK_FILES.map(file => (
                                          <div
                                                key={file.id}
                                                className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-bg-hover transition-colors group cursor-pointer focus-visible:outline-2 focus-visible:outline-accent"
                                                role="button"
                                                tabIndex={0}
                                          >
                                                ...
                                                <button
                                                      className="text-text-muted hover:text-accent focus-visible:outline-2 focus-visible:outline-accent rounded"
                        ...
                        <div className="p-4 border-t border-border bg-bg-tertiary/30">
                              <button className="w-full py-2 text-sm text-accent hover:text-accent-hover font-medium focus-visible:outline-2 focus-visible:outline-accent rounded">
                                    View All Files
                              </button>
                        </div>
                  </section>
